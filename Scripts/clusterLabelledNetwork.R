...
for(k in dir(path = "Data/",pattern = "uc", full.names = T)) {
  ...
  # plot(l2,bg=startCols,  pch=ifelse(meta$Date=="NCBI", 21,22), cex=sqrt(meta$Length)/200)
  # 
  # i="45"
  ...
  if(printPdf) {
    if(useMesh) {
      pdf("Figs/PhageNetwork_Clusters_smallSinglPosMesh.pdf", width=16,height = 16, onefile = T)
    } else {
      pdf(paste("Figs/PhageNetwork_Clusters_smallSinglPosUclust_",gsub(".*all_phages.uc(..)_def2","\\1",basename(k)),".pdf",sep=""), width=16,height = 16, onefile = T)
    }
  }
  ...
  if(runBig) {
    allPlot=plot(x = network,  
                 ...
                 #vertex.size=ifelse(meta$ClusterFix=="singleton",.1,sqrt(meta$Length)/50),                               # Size of the node (default is 15)
  ...
  #plot(l,col=clusCols, pch=16, cex=0.7)
  #plot(l2,col=clusCols, pch=16, cex=0.7)
  plot(norm_coords(l2),col=0)
  text(norm_coords(l2)[-which(meta$ClusterFix=="singleton"),],labels = meta$ClusterFix[-which(meta$ClusterFix=="singleton")],col=clusCols[-which(meta$ClusterFix=="singleton")], 
       cex=sqrt(meta$Length[-which(meta$ClusterFix=="singleton")])/200)# pch=meta$ClusterFix)
  
  pch=ifelse(meta$Date[-(meta$ClusterFix=="singleton")]=="NCBI", 21,22) 
  
  
  i=names(annot_cols$ClusterFix)[2]
  for(i in names(annot_cols$ClusterFix)) {
    
    if(i=="singleton") {
      next
    }
    clusterMeta=meta[which(meta$ClusterFix==i),]
    clustCols=clusCols[which(meta$ClusterFix==i)]
    if(NROW(clusterMeta)<4) {
      next
    }
    
    clusterCoords=norm_coords(l2)[which(meta$ClusterFix==i),]
    clusterMeans=colMeans(clusterCoords)
    plotrix::textbox(x = c(clusterMeans[1]-.05,clusterMeans[1]+.05),justify = "c",y = clusterMeans[2],fill=unique(clustCols),textlist =  unique(clusterMeta$ClusterFix),)
